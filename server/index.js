...
app.get("/app", async (req, res) => {
  ...
  Axios.get(`${baseUrl}/applications`, { headers: { Authorization: authKey } })
    ...
    .catch((err) =>
      console.error("server  >  app  > err details: ", err.message)
    );
});
...
app.get("/data", async (req, res) => {
  ...
  const [cards, trans] = await Promise.all([
    Axios.get(`${baseUrl}/cards/${appId}`, {
      headers: { Authorization: authKey },
    })
      .then(({ data }) => data)
      .catch((err) =>
        console.error("server  >  cards  > err details: ", err.message)
      ),
    Axios.get(`${baseUrl}/trans/${appId}`, {
      headers: { Authorization: authKey },
    })
      .then(({ data }) => data)
      .catch((err) =>
        console.error("server  >  trans  > err details: ", err.message)
      ),
  ]);
...
app.get("/trans", (req, res) => {
  ...
  Axios.get(`${baseUrl}/trans?appId=${appId}`, {
    headers: { Authorization: authKey },
  })
    .then(({ data }) => res.send(data))
    .catch((err) =>
